...
def constructive_bribery_from_strongest(E, T, K, P, L, B):
    F=E.remove(P)
    ...
    if dhondt_allocation(E, T, K, prefer=P)[P] >= L:
        return [E,dhondt_allocation(E, T, K, prefer=P)[P]]
    else:
        return [False]

def destructive_bribery_to_strongest(E, T, K, P, L, B):
    if E[P]<B:
        return [False]
    else:
        F = E.remove(P)
...
def constructive_bribery_from_weakest(E, T, K, P, L, B):
    F=E.remove(P)
    parties = F.parties()
    for p in parties:
        if F[p] == 0:
...
def destructive_bribery_to_weakest(E, T, K, P, L, B):
    if E[P]<B:
        return [False]
    else:
        F = E.remove(P)
