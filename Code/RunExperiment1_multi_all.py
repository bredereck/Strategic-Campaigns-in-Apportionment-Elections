...
def get_max_additional_seats_dhondt(nr,P,ind):
...
def get_max_prevented_seats_dhondt(nr,P,ind):
...
def constructive_exp_single(gainvals,nr,P):
...
def balanced_additional_dhondt(nr,P,ind):
    E_initial = election_from_file_multi(parties, votes_in_districts[nr])
    no_manipulation_in_district = dhondt_allocation(E_initial, T[nr], seats[nr], prefer=P)[P]
    ...
    results = constructive_exp_single(gainvals,nr,P)
    #print(no_manipulation)
    #print(results)
...
def destructive_exp_single(lostvals,nr,P):
    E = election_from_file_multi(parties, votes_in_districts[nr])
    year = sys.argv[1][-4:]
    results = []
    F = E.remove(P)
...
def balanced_prevented_dhondt(nr,P,ind):
    E_initial = election_from_file_multi(parties, votes_in_districts[nr])
    no_manipulation_in_district = dhondt_allocation(E_initial, T[nr], seats[nr], prefer=P)[P]
    ...
    if no_manipulation_in_district == 0:
        ...
        results = destructive_exp_single(lostvals,nr,P)
...
avg_eff_in_districts_optimal_con = []
avg_eff_in_districts_strongest_con = []
avg_eff_in_districts_weakest_con = []
avg_eff_in_districts_balanced_con = []


avg_eff_in_districts_optimal_des = []
avg_eff_in_districts_strongest_des = []
avg_eff_in_districts_weakest_des = []
avg_eff_in_districts_balanced_des = []

final_detail = open(target_dir + "all" + "-final_detail.dat", "w")
final = open(target_dir + "all" + "-final.dat", "w")
...
for P in parties:
    ...
    for nr_district in range(0,number_of_districts):
        ...
        if dhondt_allocation(E, T[nr_district], seats[nr_district], prefer=P)[P] < seats[nr_district]:
            x = get_max_additional_seats_dhondt(nr_district,P,ind)
            ...
            eff_in_districts_balanced_con.append(balanced_additional_dhondt(nr_district,P,ind))
            print("#3 done")

        if dhondt_allocation(E, T[nr_district], seats[nr_district], prefer=P)[P] > 0:
            y = get_max_prevented_seats_dhondt(nr_district,P,ind)
            ...
            eff_in_districts_balanced_des.append(balanced_prevented_dhondt(nr_district,P,ind))
            print("#4 done")

    min_optimal_con = min(eff_in_districts_optimal_con)

    avg_eff_in_districts_optimal_con.append(min_optimal_con)
    avg_eff_in_districts_strongest_con.append(min(eff_in_districts_strongest_con))
    avg_eff_in_districts_weakest_con.append(min(eff_in_districts_weakest_con))
    avg_eff_in_districts_balanced_con.append(min(eff_in_districts_balanced_con))

    if eff_in_districts_optimal_des != []:
        avg_eff_in_districts_optimal_des.append(min(eff_in_districts_optimal_des))
    else:
        eff_in_districts_optimal_des.append(-1)
        eff_votes_in_des.append(1)

    if eff_in_districts_strongest_des != []:
        avg_eff_in_districts_strongest_des.append(min(eff_in_districts_strongest_des))
    else:
        eff_in_districts_strongest_des.append(-1)

    if eff_in_districts_weakest_des != []:
        avg_eff_in_districts_weakest_des.append(min(eff_in_districts_weakest_des))
    else:
        eff_in_districts_weakest_des.append(-1)

    if eff_in_districts_balanced_des != []:
        avg_eff_in_districts_balanced_des.append(min(eff_in_districts_balanced_des))
    else:
        eff_in_districts_balanced_des.append(-1)

    min_optimal_des = min(eff_in_districts_optimal_des)

    print("%s %s %i %i %i" % (P,sum_of_votes, sum(sum_of_votes), sum_of_votes[ind], sum(sum_of_votes)-sum_of_votes[ind]),file=final_detail)

    print("%s %f %i %.10f " % ("constructive-optimal", min_optimal_con, eff_in_districts_optimal_con.index(min_optimal_con), min_optimal_con/min_optimal_con),file=final_detail)
    print("%s" % eff_in_districts_optimal_con,file=final_detail)
    print("%s %f %i %.10f " % ("constructive-strongest", min(eff_in_districts_strongest_con), eff_in_districts_strongest_con.index(min(eff_in_districts_strongest_con)),  min(eff_in_districts_strongest_con)/min_optimal_con),file=final_detail)
    print("%s " % eff_in_districts_strongest_con,file=final_detail)
    print("%s %f %i %.10f " % ("constructive-weakest", min(eff_in_districts_weakest_con), eff_in_districts_weakest_con.index(min(eff_in_districts_weakest_con)), min(eff_in_districts_weakest_con)/min_optimal_con),file=final_detail)
    print("%s " % eff_in_districts_weakest_con,file=final_detail)
    print("%s %f %i %.10f " % ("constructive-balanced", min(eff_in_districts_balanced_con), eff_in_districts_balanced_con.index(min(eff_in_districts_balanced_con)), min(eff_in_districts_balanced_con)/min_optimal_con),file=final_detail)
    print("%s " % eff_in_districts_balanced_con,file=final_detail)

    print("%s %f %i %.10f " % ("destructive-optimal", min_optimal_des, eff_in_districts_optimal_des.index(min_optimal_des), min_optimal_des/min_optimal_des),file=final_detail)
    print("%s" % eff_in_districts_optimal_des,file=final_detail)
    print("%s %f %i %.10f " % ("destructive-strongest", min(eff_in_districts_strongest_des), eff_in_districts_strongest_des.index(min(eff_in_districts_strongest_des)), min(eff_in_districts_strongest_des)/min_optimal_des),file=final_detail)
    print("%s" % eff_in_districts_strongest_des,file=final_detail)
    print("%s %f %i %.10f " % ("destructive-weakest", min(eff_in_districts_weakest_des), eff_in_districts_weakest_des.index(min(eff_in_districts_weakest_des)), min(eff_in_districts_weakest_des)/min_optimal_des),file=final_detail)
    print("%s" % eff_in_districts_weakest_des,file=final_detail)
    print("%s %f %i %.10f " % ("destructive-balanced", min(eff_in_districts_balanced_des), eff_in_districts_balanced_des.index(min(eff_in_districts_balanced_des)), min(eff_in_districts_balanced_des)/min_optimal_des),file=final_detail)
    print("%s" % eff_in_districts_balanced_des,file=final_detail)

avg_min_optimal_con = np.mean(avg_eff_in_districts_optimal_con)
avg_min_optimal_des = np.mean(avg_eff_in_districts_optimal_des)

print("%s %.10f" % ("constructive-optimal", avg_min_optimal_con/avg_min_optimal_con ),file=final)
print("%s" % avg_eff_in_districts_optimal_con,file=final)
print("%s %.10f" % ("constructive-strongest", np.mean(avg_eff_in_districts_strongest_con)/avg_min_optimal_con ),file=final)
print("%s " % avg_eff_in_districts_strongest_con,file=final)
print("%s %.10f" % ("constructive-weakest", np.mean(avg_eff_in_districts_weakest_con)/avg_min_optimal_con ),file=final)
print("%s " % avg_eff_in_districts_weakest_con,file=final)
print("%s  %.10f" % ("constructive-balanced", np.mean(avg_eff_in_districts_balanced_con)/avg_min_optimal_con ),file=final)
print("%s " % avg_eff_in_districts_balanced_con,file=final)

print("%s  %.10f" % ("destructive-optimal", avg_min_optimal_des/avg_min_optimal_des),file=final)
print("%s" % avg_eff_in_districts_optimal_des,file=final)
print("%s %.10f" % ("destructive-strongest", np.mean(avg_eff_in_districts_strongest_des)/avg_min_optimal_des),file=final)
print("%s" % avg_eff_in_districts_strongest_des,file=final)
print("%s  %.10f" % ("destructive-weakest", np.mean(avg_eff_in_districts_weakest_des)/avg_min_optimal_des),file=final)
print("%s" % avg_eff_in_districts_weakest_des,file=final)
print("%s  %.10f" % ("destructive-balanced", np.mean(avg_eff_in_districts_balanced_des)/avg_min_optimal_des),file=final)
print("%s" % avg_eff_in_districts_balanced_des,file=final)
